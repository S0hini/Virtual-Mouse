...
#initialization of mediapipe hand
...
while True:
        ...
        if result.multi_hand_landmarks:
            ...
            if dist< 0.04:
                 if not freeze_cursor:
                    ...
                    click_times.append(time.time())
                      ...
                      #double click check
                    if len(click_times)>=2 and click_times[-1]-click_times[-2]<0.6:
                        pag.doubleClick()
            ...
            if scroll_mode:
                if index_tip.y<0.4:
                    pag.scroll(60)
                    cv2.putText(frame,"Scroll Up",(10,50), cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
                elif index_tip.y>0.6:
                    pag.scroll(-60)
                    cv2.putText(frame,"Scroll Down",(10,50), cv2.FONT_HERSHEY_SIMPLEX,1,(0,255,0),2)
            #screenshot
            if sum(fingers)==0:
                current_time=time.time()
                if current_time-last_screenshot_time>screenshot_cooldown:
                    pag.screenshot(f"Screenshot_{int(current_time)}.png")
